...
const TokenContract = new web3.eth.Contract(TokenJson.abi, `0x${argv["tokenaddr"]}`);
...
module.exports = async(callback) => {
  try {
    ...
    await TokenContract.getPastEvents(eventType, {
      ...
      async(error, events) => {
        if(error) {
          ...
          for(let i = 0; i < events.length; i++) {
            ...
            console.log("address:\t" + events[i].returnValues.data);
            // Minted event data:
            // console.log("amount:\t\t" + events[i].returnValues.amount);
